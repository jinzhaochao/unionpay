...
import com.unionpay.week.controller.ServerWeekOfficeController;
...
public class MyServerServiceImpl implements MyServerService {
    ...
    @Autowired
    private ServerWeekOfficeController serverWeekOfficeService;
    @Autowired
    ...
    @Value("${http.server.completegivescore}")
    ...
    private List<ServerApplyInfo> changeToServerApplyInfo(JSONArray array){
        ...
        for(int i=0;i<array.size();i++){
            ...
            String lanchTime = jsonObject.getString("launchTime");
            serverApplyInfo.setLaunchTime(lanchTime);
            String endTime = jsonObject.getString("endTime");
            serverApplyInfo.setEndTime(endTime);
            // 增加工作日字段
            // 根据起始时间，调用流程平台接口计算工作日
            Integer workDay =0;
            ResultList list = serverWeekOfficeService.selectOffice(lanchTime,endTime);
            if (ToolUtil.isNotEmpty(list)){
                workDay = list.getPager().getTotal();
            }
            serverApplyInfo.setWorkDay(workDay);
        ...
        if (ToolUtil.isNotEmpty(serverApplyInfo)){
            ...
            if (ToolUtil.isNotEmpty(userid)){
                ...
                if (ToolUtil.isNotEmpty(receivers)&&receivers.size()>0){
                    ...
                    modelDto.setAccount(account);
                    modelDto.setPassword(password);
                    // http://p.oa.unionpay.com/serviceCenter/#/serService/service?id=
                    modelDto.setUrl(completeGiveScoreUrl+serverApplyInfo.getId());
                    modelDto.setReceivers(receivers);
                    sendNotice.sendNotice(modelDto);
                }
            }
