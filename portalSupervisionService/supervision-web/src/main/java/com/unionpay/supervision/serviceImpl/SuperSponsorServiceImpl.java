...
//                superSponsor.setProgress("");
    ...
    public List<SuperSponserPage> selectPage(String type, Integer deptId, String createTimeStart, String createTimeEnd, Integer currentPage, Integer size) {
        ...
        if (ToolUtil.isNotEmpty(type)) {
            if (Integer.parseInt(type) == 1) {
                ...
                //type=3是已中止的督办事项
                sql += " and (sp.work_status in ('工作中止') and sp.service_status = '中止') ";
                if (ToolUtil.isNotEmpty(createTimeStart)) {
                    sql += " and sp.feedback_time >= '" + createTimeStart + "' ";
                }
                if (ToolUtil.isNotEmpty(createTimeEnd)) {
    ...
    @Override
    public void del(String unids) {
        if (ToolUtil.isNotEmpty(unids)) {
            String[] unid = unids.split(",");
            for (String s : unid) {
                SuperSponsor select = superSponsorRepository.findByUnid(s);
                select.setStatus(0);
                superSponsorRepository.save(select);
            }
        }
    }

    @Override
    public void updateIsSuper(@Valid LcFeedBackDto lcFeedBackDto) {
        SuperSponsor superSponsor = superSponsorRepository.findByUnid(lcFeedBackDto.getSponsorUnid());
        superSponsor.setIsSuper(0);
        superSponsorRepository.save(superSponsor);
        List<SuperSponsorLog> sponsorLogs = superSponsorLogRepository.findBySponsorUnidOrderByCreateTimeDesc(lcFeedBackDto.getSponsorUnid());
        SuperSponsorLog superSponsorLog = sponsorLogs.get(0);
        superSponsorLog.setStatus(0);
