...
public class ServerinfoServiceImpl implements ServerInfoService{
	...
	public int countBySerId(Integer serId){
		...
		int sum = 0;
		// 已完成办件数量
		String countSql = "SELECT COUNT(id) FROM server_apply_info WHERE ser_id =:serId AND (`status` = '2' OR `status` = '3')";
		...
		// 已完成办件，花费总时长
		String sumSql = "SELECT IFNULL(SUM(TIMESTAMPDIFF(DAY, launch_time, end_time)),0)"
				+ "FROM server_apply_info WHERE ser_id =:serId AND (`status` = '2' OR `status` = '3')";
