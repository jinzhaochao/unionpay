...
//				}
                //
                //是否督办中  --jinzhao  2019-12-09
                sponsor.setIsSuper(0);
                if (ToolUtil.isNotEmpty(lcFeedBackDto.getWorkStatus())) {
                    if (Integer.parseInt(lcFeedBackDto.getWorkStatus()) == 1) {
                        //办理时间    --jinzhao  2019-12-09
                        sponsor.setThroughTime(lcFeedBackDto.getThroughTime());


                        //办理耗时  际办结时间-来源时间(如果有多个来源,按最早的那个   --jinzhao  2019-12-09
                        List<SuperServiceOverseeMapping> list = superServiceOverseeMappingRepository.findByServiceUnidOrderByServiceTime(lcFeedBackDto.getServiceUnid());
                        if (ToolUtil.isNotEmpty(list)) {
                            SuperServiceOverseeMapping superServiceOverseeMapping = list.get(0);
                            sponsor.setThroughTimes((int) (DateUtil.getDaySub(superServiceOverseeMapping.getServiceTime(), lcFeedBackDto.getThroughTime(), "yyyy-MM-dd HH:mm:ss")) + 1);
                        }
                    }
                }
