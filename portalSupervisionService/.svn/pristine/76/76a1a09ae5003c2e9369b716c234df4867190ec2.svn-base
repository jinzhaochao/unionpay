...
public interface BusinessRequireRMccRepository extends JpaRepository<BusinessRequireRMcc,Integer>, JpaSpecificationExecutor<BusinessRequireRMcc>{
     ...
     * @return
     */
    @Query(value = "SELECT m.id FROM pager_business_mcc m WHERE m.`value` IN (SELECT mcc_id from pager_business_require_r_mcc WHERE require_id =?1)",nativeQuery = true)
